...
def process_target(px4board_file, target_name, defconfig_data, protoconfig_data):
    ...
    manifest_item['description'] = protoconfig_data['description']
    manifest_item['board_id'] = protoconfig_data['board_id']
    manifest_item['summary'] = protoconfig_data['summary']
    manifest_item['image_maxsize'] = protoconfig_data['image_maxsize']
...
for manufacturer in os.scandir(os.path.join(source_dir, '../boards')):
    ...
    for board in os.scandir(manufacturer.path):
        ...
        for files in os.scandir(board.path):
            ...
            if files.is_file() and files.name.endswith('.prototype'):
                #protoconfig_file = os.path.join(board.path, 'firmware.prototype')
                protoconfig_data = process_proto(files.path)
            ...
            if files.is_file() and files.name.endswith('.px4board'):
                ...
                target = process_target(files.path,
                                        target_name,
                                        defconfig_data,
                                        protoconfig_data)
